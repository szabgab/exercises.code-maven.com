# Summary

- [Home](./index.md)

# Introduction
  - [Hello World](./exercise-hello-world.md)

# Scalars
  - [Calculate the area of a rectangula](./exercise-rectangular.md)
  - [Number guessing game](./exercise-number-guessing-game.md)
  - [Create calculator](./exercise-calculator.md)

# Files
  - [Sum of numbers in a file](./exercise-sum-of-numbers-in-file.md)
  - [Analyze Apache log file - count localhost](./exercise-analyze-apache-log-file-count-localhost.md)
  - [Add more statistics](./exercise-add-more-statistics.md)
  - [Write report to file](./exercise-write-report-to-file.md)

# Lists and Array
  - [Color selector](./exercise-color-selector.md)
  - [Add numbers taken from a CSV file](./exercise-add-numbers-from-csv-file.md)
  - [Improve the color selector](./exercise-improve-the-color-selector.md)
  - [Improve Number guessing game](./exercise-improve-number-guessing-game.md)
  - [Count digits](./exercise-count-digits.md)
  - [Sort based on secondary condition](./exercise-sort-based-on-secondary-condition.md)
  - [Display unique rows of a file](./exercise-display-unique-rows-of-a-file.md)
  - [Sort mixed string](./exercise-sort-mixed-string.md)
  - [Queue](./exercise-queue.md)
  - [Check if number is prime](./exercise-number-is-prime.md)
  - [Implement Reverse Polish Calculator](./exercise-reverse-polish-calculator.md)

# Hashes, Dictionaries
  - [Count words in a file](./exercise-count-words-in-a-file.md)
  - [Parse HTTP GET parameters](./exercise-parse-http-get-parameters.md)
  - [sort scores](./exercise-sort-scores.md)
  - [Analyze Apache log file - count every host](./exercise-analyze-apache-log-file-count-every-host.md)
  - [Parse variable width fields](./exercise-parse-variable-width-fields.md)

# Functions and Subroutines
  - [ROT13](./exercise-rot13.md)

# Recursive functions
  - [Recursive Factorial](./exercise-factorial.md)
  - [Recursive Factorial Solution](./solution-factorial.md)
  - [Recursive Fibonacci](./exercise-fibonacci.md)
  - [Recursive Fibonacci Solution](./solution-fibonacci.md)

# Regular Expressions, Regexes
  - [Regexes part 1](./exercise-regexes-part-1.md)
  - [Match numbers with regex](./exercise-match-numbers-with-regex.md)
  - [Match hexa, octal, binary numbers](./exercise-hexa-octal-binary.md)
  - [Match Roman numbers](./exercise-match-roman-numbers.md)
  - [Split file path using regex](./exercise-split-file-path-with-regex.md)
  - [Sort SNMP numbers](./exercise-sort-snmp-numbers.md)
  - [Parse hours log file and create time report](./exercise-parse-hours-log-file-and-create-time-report.md)
  - [Parse INI file](./exercise-parse-ini-file.md)

# Linux
  - [Linux as a Virtual Environment - install + nginx](./exercise-linux-as-a-virtual-environment-nginx.md)

# Simple applications.md)
  - [Implement the wc command of Linux/Unix (word count)](./exercise-implement-linux-wc.md)
  - [next](./one-dimensional-spacefight.md)
  - [next](./mastermind.md)

# Mix
  - [Send plain text e-mail](./exercise-send-plain-text-email.md)
  - [Send HTML e-mail](./exercise-send-html-email.md)
  - [Send e-mail with attachment](./exercise-send-email-with-attachment.md)
  - [next](./compare-wikipedia-translations.md)

# Databases
  - [Simple Family tree](./family-tree.md)

# Web development
  - [Hello World for Web applications](./exercise-web-hello-world.md)
  - [Echo Web application](./exercise-web-echo.md)
  - [next](./personal-bookmarks.md)
  - [next](./url-shortener.md)
  - [next](./todo.md)
  - [next](./flight-ticket-vendor.md)

# Web crawlers
  - [next](./download-a-web-page.md)
  - [next](./ted.md)

